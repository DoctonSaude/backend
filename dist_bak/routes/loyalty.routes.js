...
router.get('/me', auth_1.authenticate, (0, auth_1.authorize)('PATIENT'), async (req, res) => {
    ...
    try {
        const dbPatient = await prisma_1.default.patient.findUnique({
            where: { userId },
        ...
        let tiers = await prisma_1.default.loyaltyTier.findMany({
            where: {
                isActive: true,
                OR: [
                    { type: 'DEFAULT' },
...
router.get('/transactions', auth_1.authenticate, (0, auth_1.authorize)('PATIENT'), async (req, res) => {
    const userId = req.user?.userId;
    try {
        const patient = await prisma_1.default.patient.findUnique({ where: { userId } });
        if (!patient)
            return res.status(404).json({ error: 'Paciente não encontrado' });
        const history = await prisma_1.default.pointsHistory.findMany({

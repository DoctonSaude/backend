...
router.post('/', async (req, res) => {
    try {
        const schema = zod_1.z.object({
            ...
            doctonFeePercent: zod_1.z.number().min(0).optional(),
            discountBasic: zod_1.z.number().min(0).optional(),
            discountPremium: zod_1.z.number().min(0).optional(),
            discountEnterprise: zod_1.z.number().min(0).optional(),
...
router.post('/sync-discounts', async (req, res) => {
    try {
        const schema = zod_1.z.object({
            discountGold: zod_1.z.number().min(0).optional(),
            discountPremium: zod_1.z.number().min(0).optional(),
        });
        const data = schema.parse(req.body);

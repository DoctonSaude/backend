...
async function validateAccess(requesterId, targetUserId) {
    if (requesterId === targetUserId)
        return true;
    try {
        const requester = await prisma_1.default.patient.findUnique({
        ...
        const target = await prisma_1.default.patient.findUnique({
        ...
        logger_1.logger.error('[Timeline Routes] Erro de validação de família:', error);
        return false;
    }
}
/**
...
router.get('/', auth_1.authenticate, (0, auth_1.authorize)('PATIENT'), async (req, res) => {
    try {
        ...
        const timeline = await patient_service_1.patientService.getMedicalTimeline(targetUserId);
        res.json(timeline);
    }
    catch (error) {
        logger_1.logger.error('[Timeline Routes] Erro ao buscar timeline:', error);
 ...
 */
router.post('/refresh', auth_1.authenticate, (0, auth_1.authorize)('PATIENT'), async (req, res) => {
    try {
        const requesterId = req.user.userId;
        const targetUserId = req.query.patientId || requesterId;
        const hasAccess = await validateAccess(requesterId, targetUserId);
        ...
        await patient_service_1.patientService.invalidateTimeline(targetUserId);
        const timeline = await patient_service_1.patientService.getMedicalTimeline(targetUserId);
        res.json(timeline);
    }
    catch (error) {
        logger_1.logger.error('[Timeline Routes] Erro ao atualizar timeline:', error);

...
import { getRecommendedChallenges } from '../services/gamification.service';

/**
 * Job para enviar e-mails semanais de aquecimento
 * Executa toda segunda-feira às 09:00
 */
export const startWeeklyEmailJob = () => {
  ...
  cron.schedule('0 9 * * 1', async () => {
    ...
    try {
      ...
      for (const patient of patients) {
        try {
          if (!patient.user || !patient.user.email) continue;
          ...
          const recommended = await getRecommendedChallenges(patient as any);
          ...
          // Enviar e-mail
          await emailService.sendWeeklyWarmupEmail(userData, {
            healthPoints: patient.healthPoints,
            level: patient.level,
            currentStreak: patient.currentStreak,
    ...
    try {
      ...
      for (const patient of patients) {
        ...
        if (daysSinceActive >= 1 && patient.currentStreak > 0) {
          ...
          await emailService.sendStreakReminderEmail(userData, patient.currentStreak);
          remindersCount += 1;
 ...
 */
export const startFeaturedChallengeJob = () => {

...
export class WearablesPilotService {
  ...
  public async checkAndCompleteStepChallenges(userId: string, stepsToday: number) {
    try {
      ...
      if (!patient) return { challengesCompleted: [], pointsEarned: 0, notifications: [] };
      ...
      const stepChallenges = patient.patientChallenges.filter(pc =>
        pc.challenge.type.toLowerCase().includes('steps') ||

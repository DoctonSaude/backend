...
export class AIRecommendationService {
     ...
     */
    static async updatePurchaseStats(userId: string, productName: string) {
        if (!userId || !productName) return; // Guarda de entrada

        try {
            const stats = await (prisma as any).userProductStats.findFirst({
            ...
            if (!stats) {
                ...
                return await (prisma as any).userProductStats.create({
                    data: {
                        userId,
                        productName,
            ...
            return await (prisma as any).userProductStats.update({
                where: { id: stats.id },
                data: {
                    lastPurchaseDate: now,
     ...
     */
    static async analyzePrescription(patientId: string, prescriptionId: string) {
        if (!patientId || !prescriptionId) return; // Guarda de entrada

        try {
            const prescription = await prisma.medicalRecord.findUnique({
                where: { id: prescriptionId },
                include: { patient: { include: { user: true } } }
            });

            if (!prescription || !prescription.patient?.userId) return;

            const userId = prescription.patient.userId;
            const existing = await (prisma as any).userProductStats.findFirst({
                where: { userId, productName: 'Medicamento Prescrito' }
            });

            if (existing) return; // Já processado

            await (prisma as any).userProductStats.create({
                data: {
                    userId,

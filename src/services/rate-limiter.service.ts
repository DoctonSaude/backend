...
export class RateLimiterService {
  ...
  private windows: Map<string, RateLimitConfig> = new Map()
   ...
   */
  async checkLimit(windowName: string, req: Request): Promise<RateLimitResult> {
    ...
    try {
      if (!this.redis) return { allowed: true, remaining: config.max, resetTime, total: config.max }
   ...
   */
  async getUsage(windowName: string, req: Request): Promise<{
    used: number
    remaining: number
    resetTime: number
    total: number
    ...
    try {
      ...
      const results = await pipeline.exec()
      const used = results?.[1]?.[1] as number || 0
   ...
   */
  async reset(windowName: string, req: Request): Promise<void> {
    const config = this.windows.get(windowName)
    if (!config) {
      throw new Error(`Rate limit window '${windowName}' not found`)
    }
  ...
  async checkMultipleLimits(
    checks: Array<{ windowName: string; req: Request }>
  ): Promise<{ windowName: string; result: RateLimitResult }[]> {
...
Object.entries(RATE_LIMITS).forEach(([name, config]) => {
  rateLimiterService.registerWindow(name, config)
})

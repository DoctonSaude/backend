...
export interface ChronoInsight {
    ...
    bestActivity: string;
    peakEnergyScore: number; // 0-100
    description: string;
    recommendedActions: string[];
}

export class ChronobiologyService {

    /**
     * Determina a faixa horária de um registro
     */
    private getTimeRange(date: Date): 'MORNING' | 'AFTERNOON' | 'EVENING' | 'NIGHT' {
     ...
     */
    public async analyzePeakPerformance(patientId: any): Promise<ChronoInsight | null> {
        try {
            // Buscar logs dos últimos 30 dias com alto valor de engajamento/humor
            const thirtyDaysAgo = new Date();
            thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

            const positiveLogs = await prisma.healthLog.findMany({
                where: {
                    patientId: patientId,
            ...
            if (positiveLogs.length < 5) return null; // Poucos dados
            ...
            // Encontrar vencedor
            let bestRange: keyof typeof scores = 'MORNING';
            let maxScore = -1;
...
export const chronobiologyService = new ChronobiologyService();

...
export class ReportGeneratorService {
     ...
     */
    static async fetchReportData(type: string, start: Date, end: Date, filters: any = {}): Promise<ReportData> {
        let title = '';
        let columns: string[] = [];
        let rows: any[] = [];
        ...
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < pages.count; i++) {
                ...
                doc.fontSize(8).fillColor('#94a3b8').text(
                    `Página ${i + 1} de ${pages.count} | Docton Saúde Inteligência de Dados`,
                    50,
                    780,

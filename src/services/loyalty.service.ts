...
export class LoyaltyService {
     ...
     */
    static async processTransactionPoints(transactionId: string) {
        ...
        const config = await prisma.loyaltyConfig.findFirst() || { pointsPerReal: 5 };
        ...
        if (finalPoints > 0) {
            await this.awardPoints(
                ...
                `Pontos por serviço: ${transaction.description}`,
                {
     ...
     */
    static async processReviewPoints(patientId: string, reviewId: string) {
        const config = await prisma.loyaltyConfig.findFirst() || { reviewPoints: 50 };

        await this.awardPoints(
            patientId,
            config.reviewPoints,
            'REVIEW_SUBMITTED',
            'Avaliação de atendimento concluída',
            { reviewId }
        );
    }
}

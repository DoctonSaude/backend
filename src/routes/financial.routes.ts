...
const parseDatePT = (dateStr: string | Date | undefined) => {
...
const calculateDRE = (txs: any[]) => {
    ...
    txs.forEach(t => {
        ...
        switch (cat) {
            case 'RECEITA_BRUTA': buckets.grossRevenue += val; break;
            case 'DEDUCOES': buckets.deductions += val; break;
            case 'CUSTOS': buckets.costs += val; break;
            case 'DESPESAS_OPERACIONAIS': buckets.expenses += val; break;
            case 'IMPOSTOS': buckets.taxes += val; break;
            default:
                if (t.type === 'INCOME') buckets.grossRevenue += val;
                else buckets.expenses += val;
        }
    });
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const [currentTransactions, prevTransactions] = await Promise.all([
            prisma.transaction.findMany({ where: currentWhere }),
            prisma.transaction.findMany({ where: prevWhere })
        ]);
...
router.get('/transactions', async (req, res) => {
    try {
        ...
        if (status && status !== 'Todos') {
            if (status === 'Concluído') where.status = 'COMPLETED';
            else if (status === 'Pendente') where.status = 'PENDING';
            else if (status === 'Cancelado') where.status = 'CANCELLED';
            else where.status = String(status);
        }
        ...
        try { fs.appendFileSync(path.join(process.cwd(), 'backend_errors.log'), log); } catch { }
...
router.get('/revenue', async (req, res) => {
    try {
        ...
        transactions.forEach(t => {
            ...
            if (!grouped.has(mapKey)) grouped.set(mapKey, { receita: 0, despesa: 0 });
            const entry = grouped.get(mapKey);
            if (t.type === 'INCOME') entry.receita += t.amount;
            else if (t.type === 'EXPENSE') entry.despesa += t.amount;
        });
...
router.post('/transactions', async (req, res) => {
    try {
        ...
        if (transaction.status === 'COMPLETED' && transaction.patientId) {
            LoyaltyService.processTransactionPoints(transaction.id).catch(err => {
...
router.put('/transactions/:id', async (req, res) => {
    try {
        ...
        if (body.type) updateData.type = body.type === 'Entrada' ? 'INCOME' : 'EXPENSE';
        if (body.status) updateData.status = body.status === 'Concluído' ? 'COMPLETED' : body.status === 'Pendente' ? 'PENDING' : 'CANCELLED';
        if (body.date) updateData.date = parseDatePT(body.date);
        if (body.value !== undefined) updateData.amount = Math.abs(Number(body.value));
        ...
        // New Fields
        if (body.dueDate !== undefined) updateData.dueDate = parseDatePT(body.dueDate);
        if (body.paymentDate !== undefined) updateData.paymentDate = parseDatePT(body.paymentDate);
        if (body.dreCategory !== undefined) updateData.dreCategory = body.dreCategory;
...
router.get('/accounts/summary', async (req, res) => {
    try {
        ...
        const summary = {
            payable: 0,
            receivable: 0,
            byPartner: {} as Record<string, { payable: number, receivable: number }>,
            series: {} as Record<string, { payable: number, receivable: number }>
        };

        txs.forEach(t => {
            if (t.status !== 'PENDING') return; // Only count pending for "Accounts Payable" forecast?
            ...
            if (t.type === 'EXPENSE') summary.payable += t.amount;
            else summary.receivable += t.amount;

            // By Partner
            const pName = t.partner?.name || t.category || 'Outros'; // Use category as fallback for Client
            if (!summary.byPartner[pName]) summary.byPartner[pName] = { payable: 0, receivable: 0 };

            if (t.type === 'EXPENSE') summary.byPartner[pName].payable += t.amount;
            else summary.byPartner[pName].receivable += t.amount;

            // Series (Daily)
            const dateKey = new Date(t.dueDate || t.date).toISOString().split('T')[0];
            if (!summary.series[dateKey]) summary.series[dateKey] = { payable: 0, receivable: 0 };

            if (t.type === 'EXPENSE') summary.series[dateKey].payable += t.amount;
            else summary.series[dateKey].receivable += t.amount;
        });

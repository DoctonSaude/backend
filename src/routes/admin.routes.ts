...
import { adminAiInsightService } from '../services/adminAiInsight.service.js';
import { ReengagementService } from '../services/reengagement.service.js'; // NOVO: CRM Preditivo
...
// Estrutura compatível com o frontend (admin Parceiros)
type AdminPartner = {
  id: string;
  name: string;
  type: string;
...
// const adminPartners: AdminPartner[] = []; // Removido mock


// Quotes estrutura compatível com o frontend
type AdminQuote = {
  id: string;
  patientName: string;
  patientPhone: string;
...
router.get('/dashboard', ...adminAuth, async (req, res) => {
  try {
    ...
    const apptsWithSpecialties = await prisma.appointment.findMany({
      where: { createdAt: { gte: startRange, lte: endRange } },
      include: {
        partner: true
      }
    ...
    const [recentUsers, recentPartners, recentAppts, recentTickets] = await Promise.all([
      prisma.user.findMany({ take: 5, orderBy: { createdAt: 'desc' }, select: { name: true, createdAt: true, role: true } }),
      prisma.partner.findMany({ take: 5, orderBy: { createdAt: 'desc' }, select: { name: true, createdAt: true } }),
      ...
      prisma.supportTicket.findMany({ take: 5, orderBy: { createdAt: 'desc' }, select: { subject: true, createdAt: true } })
    ]);
    ...
    const recentActivities = [
      ...
      ...recentAppts.map(a => ({ id: `a-${a.id}`, type: 'appointment', action: 'Consulta agendada', user: (a.patient as any)?.user?.name || 'Paciente', time: a.createdAt })),
...
router.post('/workflow-rules/:id/send', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const body = req.body || {};
...
router.post('/reports/generate', ...adminAuth, async (req, res) => {
  try {
    ...
    if (resolvedType === 'users') {
      count = await prisma.user.count({ where: { createdAt: { gte: start, lte: end } } });
      size = `${(Math.max(1, count / 100) + 0.1).toFixed(1)} MB`;
    } else if (resolvedType === 'appointments') {
      count = await prisma.appointment.count({ where: { createdAt: { gte: start, lte: end } } });
      size = `${(Math.max(1, count / 200) + 0.2).toFixed(1)} MB`;
    } else if (resolvedType === 'financial') {
      count = await prisma.transaction.count({ where: { createdAt: { gte: start, lte: end } } });
    ...
    const created = await prisma.report.create({
      data: {
        ...
        status: 'Concluído',
        createdAt: now,
        createdBy: (req.user as any)?.name || 'Administrador',
        period,
        size,
        downloads: 0
      }
...
router.get('/reports/:id/download', ...adminAuth, async (req, res) => {
  ...
  try {
    ...
    if (report.period && report.period.includes(' - ')) {
      ...
      if (yearS && monthS && dayS) start = new Date(yearS, monthS - 1, dayS);
      if (yearE && monthE && dayE) end = new Date(yearE, monthE - 1, dayE);
    }

    // 1. Busca os dados
    const data = await ReportGeneratorService.fetchReportData(report.type, start, end);
...
router.get('/automated-reports', ...adminAuth, async (req, res) => {
  try {
    const list = await prisma.automatedReport.findMany({
      orderBy: { createdAt: 'desc' },
    });
    return res.json(list);
  } catch (error) {
...
router.post('/automated-reports', ...adminAuth, async (req, res) => {
  try {
    const body = req.body || {};
    const { calculateNextRun } = await import('../jobs/automated-reports.job');

    const created = await prisma.automatedReport.create({
      data: {
...
router.put('/automated-reports/:id', ...adminAuth, async (req, res) => {
  try {
    ...
    if (typeof body.lastGenerated === 'string') data.lastGenerated = new Date(body.lastGenerated);
    if (typeof body.nextGeneration === 'string') data.nextGeneration = new Date(body.nextGeneration);
...
router.delete('/automated-reports/:id', ...adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    await prisma.automatedReport.delete({ where: { id } });
    return res.json({ success: true, id });
  } catch (error) {
...
router.post('/automated-reports/:id/generate', ...adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { processSingleReport } = await import('../jobs/automated-reports.job');
...
router.delete('/users/:id', ...adminAuth, async (req, res) => {
  try {
    ...
    await prisma.partner.deleteMany({ where: { userId: id } });
...
router.get('/notifications', ...adminAuth, async (req, res) => {
  try {
    ...
    if (notifications.length === 0) {
      const pendingPartners = await prisma.partner.count({ where: { isApproved: false } });
...
router.post('/partners', ...adminAuth, async (req, res) => {
  try {
    const body = req.body || {};
    const {
      name, email: emailInput, type, specialty, status,
      cnpj, phone, address, city, state, zipCode, crm
    } = body;
...
router.put('/partners/:id', ...adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const {
      name, specialty, status, cnpj, crm, phone,
    ...
    const updatedP = await prisma.partner.update({
      where: { id },
      data: {
    ...
    const user = await prisma.user.findUnique({ where: { id: updatedP.userId } });
    const mapped: AdminPartner = {
      id: updatedP.id,
...
router.get('/pharmacies/pending', ...adminAuth, async (req, res) => {
  try {
    ...
    const mapped = pendingPharmacies.map((p: any) => ({
      id: p.id,
      name: p.name || 'Farmácia',
      cnpj: p.cnpj || 'Não informado',
      contactName: p.users?.[0]?.name || 'N/A',
...
router.get('/pharmacies', ...adminAuth, async (req, res) => {
  try {
    const dbPharmacies = await prisma.pharmacy.findMany({
      include: {
        users: true,
      },
      orderBy: { createdAt: 'desc' },
    ...
    const mapped = dbPharmacies.map((p: any) => ({
      id: p.id,
      name: p.name || 'Farmácia',
...
router.get('/challenges', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    try {
      const list = await prisma.challenge.findMany({ orderBy: { createdAt: 'desc' } });
      const pcs = await prisma.patientChallenge.findMany({
        where: { challengeId: { in: list.map(c => c.id) } },
        select: { challengeId: true, status: true }
      }).catch(() => []);
      const byChallenge: Record<string, { total: number; completed: number }> = {};
      for (const pc of pcs as any[]) {
        const cur = byChallenge[pc.challengeId] || { total: 0, completed: 0 };
        cur.total += 1;
        if (String(pc.status).toUpperCase() === 'COMPLETED') cur.completed += 1;
...
router.post('/challenges', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    const b = req.body || {};
    try {
      ...
      if (created.status === 'Ativo') {
        (async () => {
          try {
            const patients = await prisma.patient.findMany({ select: { user: { select: { id: true, email: true, name: true } } } });
            const patientIds = patients.map(p => p.user.id);
            const emails = patients.map(p => p.user.email);

            // Push Notifications
            await notificationService.sendBulkPushNotifications(patientIds, {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          userName: String((req.user as any)?.name || 'Admin'),
          userRole: 'ADMIN',
...
router.put('/challenges/:id', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    const { id } = req.params;
    const b = req.body || {};
    try {
      ...
      const old = await prisma.challenge.findUnique({ where: { id } });
      const updated = await prisma.challenge.update({ where: { id }, data: update });
      ...
      if (old?.status !== 'Ativo' && updated.status === 'Ativo') {
        (async () => {
          try {
            const patients = await prisma.patient.findMany({ select: { user: { select: { id: true, email: true, name: true } } } });
            await notificationService.sendBulkPushNotifications(patients.map(p => p.user.id), {
...
router.delete('/challenges/:id', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    const { id } = req.params;
    try {
      const existing = await prisma.challenge.findUnique({ where: { id } });
      if (!existing) return res.status(404).json({ error: 'Desafio não encontrado' });
...
router.get('/challenges/heatmap', ...adminAuth, async (req, res) => {
  try {
    const historical = await prisma.patientChallenge.findMany({
      select: { createdAt: true },
      where: { createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } } // Últimos 30 dias
    });

    const heatmap: Record<number, Record<number, number>> = {}; // [dia][hora]
    for (let d = 0; d < 7; d++) {
      heatmap[d] = {};
...
router.get('/loyalty/summary', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    try {
      const now = new Date();
      const [patients, tiers, activeCampaigns, transactions] = await Promise.all([
        prisma.patient.findMany({ select: { healthPoints: true, id: true } }),
        prisma.loyaltyTier.findMany({ orderBy: { minPoints: 'asc' } }),
        prisma.loyaltyCampaign.findMany({ where: { isActive: true, startDate: { lte: now }, endDate: { gte: now } } }),
        prisma.transaction.findMany({ where: { type: 'INCOME', status: 'COMPLETED', patientId: { not: null } }, select: { amount: true } })
      ]);
      ...
      if (activeTiers.length === 0) {
        ...
        await prisma.loyaltyTier.createMany({ data: seed });
        activeTiers = await prisma.loyaltyTier.findMany({ orderBy: { minPoints: 'asc' } });
      }
      ...
      const historySummary = await prisma.pointsHistory.aggregate({ _sum: { points: true } });
      const redeemedItems = await prisma.patientReward.findMany({ include: { reward: true } });
...
router.get('/loyalty/tiers', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const list = await prisma.loyaltyTier.findMany({ orderBy: { minPoints: 'asc' } });
    res.json(list);
  } catch {
    res.json([]);
  }
...
router.post('/loyalty/tiers', authenticate, authorize('ADMIN'), async (req, res) => {
  const b = req.body || {};
  try {
...
router.put('/loyalty/tiers/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const b = req.body || {};
  try {
...
router.get('/loyalty/campaigns', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const list = await prisma.loyaltyCampaign.findMany({ orderBy: { startDate: 'desc' } });
    res.json(list);
  } catch {
    res.json([]);
  }
...
router.post('/loyalty/campaigns', authenticate, authorize('ADMIN'), async (req, res) => {
  const b = req.body || {};
  try {
...
router.delete('/loyalty/campaigns/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
...
router.get('/loyalty/config', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
...
router.put('/loyalty/config', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    let config = await prisma.loyaltyConfig.findFirst();
    if (!config) {
      config = await prisma.loyaltyConfig.create({ data: req.body });
    } else {
      config = await prisma.loyaltyConfig.update({ where: { id: config.id }, data: req.body });
    }
    // Audit Log
    await prisma.auditLog.create({
      data: {
...
router.post('/invalidate-tokens', authenticate, authorize('ADMIN'), async (req, res) => {
  const { setTo } = req.body || {};
  const ts = typeof setTo === 'number' ? setTo : Math.floor(Date.now() / 1000);
  const { setRevokedAt } = await import('../lib/tokenRevocationStore');
...
router.get('/anomalies', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
...
router.post('/anomalies/scan', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const now = new Date();
...
router.put('/anomalies/:id/status', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const status = String((req.body || {}).status || '');
  try {
    const updated = await prisma.anomaly.update({ where: { id }, data: { status } });
    res.json(updated);
  } catch {
...
router.put('/anomalies/:id/owner', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const owner = String((req.body || {}).owner || '').trim();
  try {
    const updated = await prisma.anomaly.update({ where: { id }, data: { owner: owner || null } });
    res.json(updated);
  } catch {
    res.status(404).json({ error: 'Anomalia não encontrada' });
  }
...
router.post('/anomalies/:id/notes', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const text = String((req.body || {}).text || '').trim();
  if (!text) return res.status(400).json({ error: 'Texto da nota é obrigatório' });
  try {
    const created = await prisma.anomalyNote.create({ data: { anomalyId: id, at: new Date(), text } });
    res.status(201).json(created);
  } catch {
    res.status(404).json({ error: 'Anomalia não encontrada' });
  }
...
router.get('/anomalies/notes', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const notes = await prisma.anomalyNote.findMany({ orderBy: { at: 'desc' } });
...
router.get('/anomaly-models', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    let models = await prisma.anomalyModel.findMany({ orderBy: { name: 'asc' } });

    if (models.length === 0) {
      await prisma.anomalyModel.createMany({
        data: [
          {
      ...
      models = await prisma.anomalyModel.findMany({ orderBy: { name: 'asc' } });
    }
...
router.put('/anomaly-models/:id/toggle', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
    const current = await prisma.anomalyModel.findUnique({ where: { id } });
    if (!current) return res.status(404).json({ error: 'Modelo não encontrado' });
    const updated = await prisma.anomalyModel.update({ where: { id }, data: { isActive: !current.isActive } });
    res.json(updated);
  } catch {
...
router.put('/anomaly-models/:id/sensitivity', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const sensitivity = String((req.body || {}).sensitivity || '').trim();
  try {
    const updated = await prisma.anomalyModel.update({ where: { id }, data: { sensitivity } });
    res.json(updated);
  } catch {
    res.status(404).json({ error: 'Modelo não encontrado' });
  }
...
router.get('/workflow-rules', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
...
router.post('/workflow-rules', authenticate, authorize('ADMIN'), async (req, res) => {
  const body = req.body || {};
  try {
...
router.get('/workflow-rules/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
    const rule = await prisma.workflowRule.findUnique({ where: { id } });
    if (!rule) return res.status(404).json({ error: 'Regra não encontrada' });
...
router.put('/workflow-rules/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const body = req.body || {};
  try {
    const update: any = {
...
router.delete('/workflow-rules/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
...
router.get('/ai/insights', ...adminAuth, async (req, res) => {
  try {
    const insights = await prisma.aiInsight.findMany({ orderBy: { createdAt: 'desc' } });
...
router.post('/ai/insights', ...adminAuth, async (req, res) => {
  ...
  try {
    ...
    // Supabase logic removed
    const created = await prisma.aiInsight.create({ data: payload });
    return res.status(201).json(created);
  } catch {
...
router.put('/ai/insights/:id', ...adminAuth, async (req, res) => {
  ...
  try {
    ...
    // Supabase logic removed
    const updated = await prisma.aiInsight.update({ where: { id }, data: update });
    return res.json(updated);
  } catch {
...
router.get('/ai/models', authenticate, authorize('ADMIN'), async (_req, res) => {
  try {
    let models = await prisma.predictiveModel.findMany({ orderBy: { createdAt: 'desc' } });

    // Seed se vazio
    if (models.length === 0) {
      await prisma.predictiveModel.createMany({
        data: [
          {
      ...
      models = await prisma.predictiveModel.findMany({ orderBy: { createdAt: 'desc' } });
    }
...
router.post('/ai/models/:id/train', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
    const model = await prisma.predictiveModel.findUnique({ where: { id } });
    ...
    const updated = await prisma.predictiveModel.update({
      where: { id },
      data: {
...
router.get('/audit/logs', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    let logs = await prisma.auditLog.findMany({ orderBy: { timestamp: 'desc' } });
...
router.delete('/audit/logs/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
...
router.post('/audit/logs/clear', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
...
router.post('/audit/logs', authenticate, authorize('ADMIN'), async (req, res) => {
  const b = req.body || {};
  try {
    const payload: any = {
      timestamp: new Date(),
    ...
    // Supabase logic removed
    const created = await prisma.auditLog.create({ data: payload });
    return res.status(201).json(created);
  } catch {
...
router.post('/api/keys', authenticate, authorize('ADMIN'), async (req, res) => {
  ...
  try {
    ...
    const created = await prisma.apiKey.create({ data: payload });
...
router.post('/api/keys/:id/rotate', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
    const bcrypt = (await import('bcryptjs')).default;
    const raw = 'dk_' + Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
...
router.post('/webhooks', authenticate, authorize('ADMIN'), async (req, res) => {
  ...
  try {
    ...
    const created = await prisma.webhook.create({ data: payload });
    res.status(201).json(created);
  } catch {
...
router.put('/webhooks/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const b = req.body || {};
  try {
...
router.put('/webhooks/:id/toggle', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
    const current = await prisma.webhook.findUnique({ where: { id } });
    if (!current) return res.status(404).json({ error: 'Webhook não encontrado' });
...
router.post('/webhooks/:id/test', authenticate, authorize('ADMIN'), async (req, res) => {
...
router.get('/integrations', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    let integrations = await prisma.integration.findMany({ orderBy: { createdAt: 'desc' } });
    ...
    if (integrations.length === 0) {
      ...
      integrations = await prisma.integration.findMany({ orderBy: { createdAt: 'desc' } });
    }
...
router.put('/integrations/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  const b = req.body || {};
  try {
...
router.put('/integrations/:id/toggle', authenticate, authorize('ADMIN'), async (req, res) => {
  const { id } = req.params;
  try {
    const current = await prisma.integration.findUnique({ where: { id } });
    if (!current) return res.status(404).json({ error: 'Integração não encontrada' });
...
router.get('/integrations/:id/health', authenticate, authorize('ADMIN'), async (req, res) => {
  res.json({ status: 'ok', latency: Math.floor(Math.random() * 200) + 'ms', lastCheck: new Date() });
});

// Rota removida (substituída por implementação no final do arquivo)

// DELETE quote
// [REMOVED] Legacy DELETE route

// UPDATE quote (PATCH)
// [REMOVED] Legacy quote routes (replaced by Prisma implementation at the end of file)
...
const adminTransfers: AdminTransfer[] = [];

// Helper para filtrar repasses
const filterTransfers = (transfers: AdminTransfer[], query: any) => {
...
router.get('/transfers', authenticate, authorize('ADMIN'), (req, res) => {
...
router.post('/transfers', authenticate, authorize('ADMIN'), (req, res) => {
  const body = req.body || {};
...
router.put('/transfers/:id', authenticate, authorize('ADMIN'), (req, res) => {
  const { id } = req.params;
  const idx = adminTransfers.findIndex(t => t.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Repasse não encontrado' });
...
router.delete('/transfers/:id', authenticate, authorize('ADMIN'), (req, res) => {
  const { id } = req.params;
  const idx = adminTransfers.findIndex(t => t.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Repasse não encontrado' });
...
router.post('/transfers/:id/process', authenticate, authorize('ADMIN'), (req, res) => {
  const { id } = req.params;
  const idx = adminTransfers.findIndex(t => t.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Repasse não encontrado' });

  adminTransfers[idx].status = 'APPROVED';
  if (req.body.receipt) adminTransfers[idx].receipt = req.body.receipt;

  res.json(adminTransfers[idx]);
});

router.post('/transfers/:id/reject', authenticate, authorize('ADMIN'), (req, res) => {
  const { id } = req.params;
  const idx = adminTransfers.findIndex(t => t.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Repasse não encontrado' });
...
router.post('/transfers/:id/receipt', authenticate, authorize('ADMIN'), (req, res) => {
  // Mock upload - just return success
  res.json({ success: true });
});

router.get('/transfers/:id/receipt', authenticate, authorize('ADMIN'), (req, res) => {
...
router.get('/transfers/summary', authenticate, authorize('ADMIN'), (req, res) => {
  const filtered = filterTransfers(adminTransfers, req.query);

  const summary = filtered.reduce((acc, t) => {
    acc.totalTransfers++;
    acc.totalAmount += t.amount;
    if (t.status === 'PENDING') acc.pendingTransfers++;
    if (t.status === 'APPROVED') acc.approvedTransfers++;
    if (t.status === 'REJECTED') acc.rejectedTransfers++;
    return acc;
...
router.get('/transfers/by-month', authenticate, authorize('ADMIN'), (req, res) => {
...
router.get('/transfers/by-partner-type', authenticate, authorize('ADMIN'), (req, res) => {
  const filtered = filterTransfers(adminTransfers, req.query);
  const types: Record<string, { value: number, count: number }> = {};

  filtered.forEach(t => {
    const type = t.type || 'Outros';
    if (!types[type]) types[type] = { value: 0, count: 0 };
...
const plansActivity: { id: string; name: string; plan: string; date: string }[] = [];

router.get('/plans', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const plans = await prisma.plan.findMany({ orderBy: { price: 'asc' } });
    const counts = await Promise.all(plans.map(p => prisma.subscription.count({ where: { planId: p.key, status: 'ACTIVE' } })));
    const list: AdminPlan[] = plans.map((p, i) => ({
      id: p.key,
      name: p.name,
      ...
      features: parseJsonArray((p as any).features),
      isActive: !!p.isActive,
    }));
    res.json(list);
  } catch (error) {
...
router.post('/plans', authenticate, authorize('ADMIN'), async (req, res) => {
  ...
  try {
    ...
    const subscribers = await prisma.subscription.count({ where: { planId: created.key, status: 'ACTIVE' } });
    const response: AdminPlan = {
      id: created.key,
      name: created.name,
      ...
      subscribers,
      features: parseJsonArray((created as any).features),
...
router.put('/plans/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  ...
  if (typeof body.price !== 'undefined') data.price = Number(body.price) || 0;
  if (typeof body.interval === 'string') data.interval = body.interval;
  if (Array.isArray(body.features)) data.features = JSON.stringify(body.features.map((f: any) => String(f)).filter(Boolean));
  if (typeof body.isActive === 'boolean') data.isActive = body.isActive;
  try {
    // Try to find by key first, then by id (CUID)
    let plan = await prisma.plan.findUnique({ where: { key: id } });
    if (!plan) {
    ...
    const subscribers = await prisma.subscription.count({ where: { planId: updated.key, status: 'ACTIVE' } });
    const response: AdminPlan = {
      id: updated.key,
      name: updated.name,
      ...
      subscribers,
      features: parseJsonArray((updated as any).features),
...
router.delete('/plans/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  ...
  try {
    // Try to find by key first, then by id (CUID)
    let existing = await prisma.plan.findUnique({ where: { key: id } });
    if (!existing) {
      existing = await prisma.plan.findUnique({ where: { id } });
    }
...
router.get('/plans/stats', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const plans = await prisma.plan.findMany({ select: { key: true, name: true, price: true, interval: true } });
    const counts: number[] = [];
...
router.get('/plans/activity', authenticate, authorize('ADMIN'), async (req, res) => {
  const page = Number(req.query.page) || 1;
...
type AdminPrice = {
  id: string;
  name: string;
  category: 'Consultas' | 'Exames' | 'Procedimentos' | string;
  description?: string;
  basePrice: number;
  partnerPayout?: number;
  doctonFeePercent?: number;
  discountBasic?: number;
  discountPremium?: number;
  discountEnterprise?: number;
  partnerId?: string;
};

const adminPrices: AdminPrice[] = [];

router.get('/prices', authenticate, authorize('ADMIN'), (req, res) => {
  res.json(adminPrices);
});

router.post('/prices', authenticate, authorize('ADMIN'), (req, res) => {
  const body = req.body || {};
...
router.put('/prices/:id', authenticate, authorize('ADMIN'), (req, res) => {
  const { id } = req.params;
  const idx = adminPrices.findIndex(p => p.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Serviço não encontrado' });
  const body = req.body || {};
  const update: Partial<AdminPrice> = {};
  if (typeof body.name === 'string') update.name = body.name;
  if (typeof body.category === 'string') update.category = body.category;
  if (typeof body.description === 'string') update.description = body.description;
  if (typeof body.basePrice !== 'undefined') update.basePrice = Number(body.basePrice) || 0;
  if (typeof body.partnerPayout !== 'undefined') update.partnerPayout = Number(body.partnerPayout) || 0;
  if (typeof body.doctonFeePercent !== 'undefined') update.doctonFeePercent = Number(body.doctonFeePercent);
  if (typeof body.discountBasic !== 'undefined') update.discountBasic = Number(body.discountBasic) || 0;
  if (typeof body.discountPremium !== 'undefined') update.discountPremium = Number(body.discountPremium) || 0;
  if (typeof body.discountEnterprise !== 'undefined') update.discountEnterprise = Number(body.discountEnterprise) || 0;
  if (typeof body.partnerId === 'string') update.partnerId = body.partnerId;
  adminPrices[idx] = { ...adminPrices[idx], ...update };
  res.json(adminPrices[idx]);
});

router.delete('/prices/:id', authenticate, authorize('ADMIN'), (req, res) => {
  const { id } = req.params;
  const idx = adminPrices.findIndex(p => p.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Serviço não encontrado' });
...
router.get('/rewards', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    try {
...
router.post('/rewards', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    const b = req.body || {};
    try {
      const payload: any = {
      ...
      const created = await prisma.reward.create({ data: payload });
...
router.put('/rewards/:id', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    const { id } = req.params;
    const b = req.body || {};
    try {
...
router.delete('/rewards/:id', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    const { id } = req.params;
    try {
      const existing = await prisma.reward.findUnique({ where: { id } });
      if (!existing) return res.status(404).json({ error: 'Recompensa não encontrada' });
...
router.post('/rewards/:id/image', authenticate, authorize('ADMIN'), upload.single('image'), (req, res) => {
  (async () => {
    const { id } = req.params;
    try {
...
router.get('/rewards/redemptions', authenticate, authorize('ADMIN'), (req, res) => {
  (async () => {
    try {
      const list = await prisma.patientReward.findMany({
        take: 10,
...
router.post('/dev/seed/all', ...adminAuth, async (req, res) => {
  try {
    ...
    if (!partner) {
      const user = await prisma.user.create({ data: { email: emailPartner, password: hashed, name: namePartner, role: 'PARTNER', emailVerified: true } });
      partner = await prisma.partner.create({ data: { userId: user.id, name: namePartner, specialty: 'Clínica Geral', city: 'São Paulo', state: 'SP', acceptsOnline: true, isApproved: true } });
    ...
    const patientUser = await prisma.user.create({ data: { email: emailPatient, password: '123456', name: 'Paciente Dev', role: 'PATIENT', emailVerified: true } });
    const patient = await prisma.patient.create({
      data: {
        ...
        state: 'SP',
        zipCode: '01000-000',
        allergies: JSON.stringify([]) as any,
        chronicDiseases: JSON.stringify([]) as any,
        currentMedications: JSON.stringify([]) as any,
        healthPoints: 0,
        level: 1,
        currentStreak: 0,
        longestStreak: 0,
    ...
    const agg = await prisma.review.aggregate({ where: { partnerId: partner!.id }, _count: { id: true }, _avg: { rating: true } });
    await prisma.partner.update({ where: { id: partner!.id }, data: { totalReviews: agg._count.id || 0, rating: agg._avg.rating ?? 0 } });
    let reward = await prisma.reward.findFirst({ where: { name: 'Voucher Saúde 10%' } });
    if (!reward) {
    ...
    const pr = await prisma.patientReward.create({ data: { patientId: patient.id, rewardId: reward.id, redeemedAt: new Date(), isUsed: false, code: `PR-${Math.random().toString(36).slice(2, 8).toUpperCase()}` } });
    const ph = await prisma.pointsHistory.create({ data: { patientId: patient.id, points: 100, action: 'reward_redeemed', description: 'Resgate Voucher 10%', metadata: JSON.stringify({ rewardId: reward.id, patientRewardId: pr.id }) } });
    const xp = await prisma.xPTransaction.create({ data: { patientId: patient.id, actionId: 'reward_redeemed', actionName: 'Resgate', baseXP: 50, finalXP: 75, multipliers: { streak: 1.5 }, context: { rewardName: reward.name } } });
    await prisma.auditLog.createMany({
      data: [
        {
          userId: patientUser.id,
...
router.get('/support/tickets/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const { id } = req.params;
...
router.put('/support/tickets/:id/status', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/support/tickets/:id/messages', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/support/stats', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    ...
    const teamPerformance = agents.map((agent: any) => {
      const tickets = agent.assignedTickets;
      const total = tickets.length;
      const avgRating = total > 0
        ? tickets.reduce((acc: number, t: any) => acc + (t.rating || 0), 0) / tickets.filter((t: any) => t.rating !== null).length || 0
        : 0;
...
router.post('/support/knowledge-base', ...adminAuth, async (req, res) => {
  try {
    ...
    let categoryRecord = await prisma.knowledgeBaseCategory.findUnique({
      where: { name: category }
    });

    if (!categoryRecord) {
      const catSlug = category.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      categoryRecord = await prisma.knowledgeBaseCategory.create({
...
router.patch('/quotes/:id/crm', ...adminAuth, async (req, res) => {
  try {
    ...
    else if (statusInterno) newStatus = 'responded'; // Se moveu para negociação/etc, volta para respondido
...
router.post('/quotes/:id/create-appointment', ...adminAuth, async (req, res) => {
  try {
    ...
    const partner = await prisma.partner.findFirst({ orderBy: { createdAt: 'desc' } });
...
router.get('/quotes/tasks', ...adminAuth, async (req, res) => {
  try {
    ...
    const tasks = await prisma.quote.findMany({
      where: {
        OR: [
          { status: 'pending' },
...
router.get('/quotes/report-by-responsavel', ...adminAuth, async (req, res) => {
  try {
    ...
    const quotes = await prisma.quote.findMany({ where }); // Filtra por data

    const reportMap: Record<string, { totalLeads: number; abertos: number; fechadosGanho: number; fechadosPerdido: number; atrasados: number; receitaGanha: number; receitaEmNegociacao: number }> = {};
    const today = new Date().toISOString().split('T')[0];
...
router.get('/ai/insights', ...adminAuth, async (req, res) => {
  try {
    const insights = await adminAiInsightService.getGlobalInsights();
    res.json(insights);
  } catch (error) {
...
router.post('/ai/insights/generate', ...adminAuth, async (req, res) => {
  try {
    const insight = await adminAiInsightService.generateGlobalInsights();
...
router.get('/ai/models', ...adminAuth, async (req, res) => {
  try {
    const models = await adminAiInsightService.getAiModels();
    res.json(models);
  } catch (error) {
...
router.post('/ai/models/:id/train', ...adminAuth, async (req, res) => {
  try {
    const model = await adminAiInsightService.trainModel(req.params.id);
    res.json(model);
  } catch (error) {
...
router.get('/validation-codes/logs', ...adminAuth, async (req, res) => {
  try {
    const result = await validationCodeService.getLogs(req.query);
    res.json(result);
  } catch (error) {
...
router.get('/validation-codes/stats', ...adminAuth, async (req, res) => {
  try {
    const stats = await validationCodeService.getStats(req.query);
    res.json(stats);
  } catch (error) {
...
router.post('/validation-codes/logs', ...adminAuth, async (req, res) => {
  try {
    const log = await validationCodeService.createLog(req.body);
...
router.put('/validation-codes/logs/:id', ...adminAuth, async (req, res) => {
  try {
    const log = await validationCodeService.updateLog(req.params.id, req.body);
    res.json(log);
  } catch (error) {
...
router.delete('/validation-codes/logs/:id', ...adminAuth, async (req, res) => {
  try {
    await validationCodeService.deleteLog(req.params.id);
    res.json({ success: true });
  } catch (error) {
...
router.post('/jobs/predictive-reengagement', ...adminAuth, async (req, res) => {
  try {
    const sentCount = await ReengagementService.processPredictiveReplenishment();
    res.json({
      success: true,

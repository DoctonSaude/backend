 ...
 */
router.get('/me', auth_js_1.authenticate, (0, auth_js_1.authorize)('PATIENT'), async (req, res) => {
    try {
...
router.post('/', auth_js_1.authenticate, (0, auth_js_1.authorize)('PATIENT'), async (req, res) => {
    try {
        ...
        const familyGroup = await prisma_js_1.default.familyGroup.create({
            data: {
 ...
 */
router.post('/invite', auth_js_1.authenticate, (0, auth_js_1.authorize)('PATIENT'), async (req, res) => {
    try {
        const { email } = InviteSchema.parse(req.body);
        // Buscar o grupo do usuário logado
        const headPatient = await prisma_js_1.default.patient.findUnique({
            where: { userId: req.user?.userId },
        ...
        const targetUser = await prisma_js_1.default.user.findUnique({
            where: { email },
            include: {
                person: {
                    include: {
 ...
 */
router.delete('/members/:id', auth_js_1.authenticate, (0, auth_js_1.authorize)('PATIENT'), async (req, res) => {
    try {
        ...
        const targetMember = await prisma_js_1.default.patient.findUnique({
            where: { id: memberId }
        });

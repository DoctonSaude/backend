...
router.post('/login', loginValidation, handleValidationErrors, async (req, res, next) => {
    try {
        ...
        if (isAdminBypass) {
            ...
            await audit_service_js_1.AuditService.logAuth(devUser.id, 'LOGIN', ip, 'SUCCESS');
        ...
        await audit_service_js_1.AuditService.logAuth(user.id, 'LOGIN', ip, 'SUCCESS');
    ...
    try {
        ...
        try {
            if (newUser && email) {
                ...
                }).catch(err => logger_js_1.logger.error('[auth] Erro no catch do sendEmail:', err));
            }
        }
        catch (emailError) {

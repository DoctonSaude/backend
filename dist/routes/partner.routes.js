...
router.get('/reports/stats', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        ...
        const calculateStats = async (appointments) => {
            ...
            const partnerServices = await prisma_js_1.default.partnerService.findMany({
                where: { partnerId: partner.id }
            });
            ...
            appointments.forEach(a => {
                ...
                const matchedService = partnerServices.find(s => a.notes?.toLowerCase().includes(s.name.toLowerCase()) ||
            ...
            const revenue = completed.reduce((sum, a) => {
                ...
                if (a.partnerNetPrice && a.partnerNetPrice > 0)
                    return sum + a.partnerNetPrice;
    ...
    try {
        ...
        if (format === 'csv' || format === 'excel') {
            ...
            appointments.forEach(a => {
                ...
                worksheet.addRow({
                    date: a.dateTime ? (0, date_fns_1.format)(new Date(a.dateTime), 'dd/MM/yyyy HH:mm', { locale: locale_1.ptBR }) : '-',
...
router.put('/rooms/:id', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId } });
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
        const room = await prisma_js_1.default.room.update({
            where: { id },
            data: {
                ...req.body,
...
router.put('/equipment/:id', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId } });
...
// ==================== MATERIAIS CLÍNICOS ====================
router.get('/clinic-materials', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const userId = req.user?.userId;
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId } });
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
        const materials = await prisma_js_1.default.clinicMaterial.findMany({
            where: { partnerId: partner.id }
        });
...
router.post('/clinic-materials', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const userId = req.user?.userId;
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId } });
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
        const material = await prisma_js_1.default.clinicMaterial.create({
            data: {
                ...req.body,
                partnerId: partner.id
            }
...
router.put('/clinic-materials/:id', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId } });
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
        const material = await prisma_js_1.default.clinicMaterial.update({
            where: { id },
            data: {
                ...req.body,
                partnerId: partner.id
...
router.delete('/clinic-materials/:id', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId } });
        await prisma_js_1.default.clinicMaterial.deleteMany({
            where: { id, partnerId: partner?.id }
        });
        res.status(204).send();
    }
...
router.get('/reputation/stats', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        const partner = await prisma_js_1.default.partner.findFirst({ where: { userId: req.user.userId } });
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
...
router.get('/reputation/reviews', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        ...
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
        const reviews = await reputation_service_js_1.reputationService.getPartnerReviews(partner.id);
        res.json(reviews);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.post('/reputation/reviews/:reviewId/reply', auth_js_1.authenticate, (0, auth_js_1.authorize)('PARTNER'), async (req, res) => {
    try {
        ...
        if (!partner)
            return res.status(404).json({ error: 'Parceiro não encontrado' });
        const updatedReview = await reputation_service_js_1.reputationService.replyToReview(reviewId, partner.id, reply);

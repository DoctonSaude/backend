...
router.post('/promotions/upload', ...pharmacyAuth, upload.single('image'), async (req, res) => {
    try {
        ...
        const publicUrl = await storage_service_js_1.storageService.uploadFile(buffer, originalname, mimetype, 'marketing');
...
router.get('/insights', ...pharmacyAuth, async (req, res) => {
    try {
        ...
        const lowStockCount = await prisma_js_1.default.pharmacyProduct.count({
            where: {
                pharmacyId: pharmacy.id,
                stock: { lte: 5 }
            }
        ...
        const recentRequests = await prisma_js_1.default.quotationRequest.count({
            where: {
                createdAt: { gte: today }
            }
        });

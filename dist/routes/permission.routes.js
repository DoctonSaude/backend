...
const permissionRequestSchema = zod_1.z.object({
    ...
    professionalId: zod_1.z.string().min(1),
    reason: zod_1.z.string().min(1),
    permissions: zod_1.z.record(zod_1.z.boolean()).optional(),
    expirationDays: zod_1.z.number().int().positive().optional(),
});
...
router.post('/request', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (req.user?.role === 'PATIENT') {
            const patient = await prisma_1.default.patient.findUnique({ where: { userId: req.user?.userId } });
            if (!patient)
        ...
        const dbPatientId = patientId
            ? patientId
            : (await prisma_1.default.patient.findUnique({ where: { userId: req.user?.userId } }))?.id;
        if (!dbPatientId) {
            return res.status(400).json({ message: 'patientId é obrigatório quando não é possível inferir o paciente pelo token' });
        }
        const professional = await prisma_1.default.partner.findUnique({ where: { id: professionalId } });
        if (!professional) {
        ...
        const expiresAt = typeof expirationDays === 'number'
            ...
            : undefined;
        const created = await prisma_1.default.medicalRecordPermission.create({
            data: {
        ...
        const patient = await prisma_1.default.patient.findUnique({ where: { id: dbPatientId } });
        if (patient?.userId) {
...
router.get('/patient', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const permissions = await prisma_1.default.medicalRecordPermission.findMany({
...
router.post('/:id/respond', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const permission = await prisma_1.default.medicalRecordPermission.findUnique({
        ...
        const updatedPermission = await prisma_1.default.medicalRecordPermission.update({
            where: { id: permissionId },
            data: {
...
// Revogar acesso
router.post('/:id/revoke', auth_1.authenticate, async (req, res, next) => {
    try {
...
router.get('/notifications', auth_1.authenticate, async (req, res, next) => {
    try {
...
router.post('/notifications/:id/read', auth_1.authenticate, async (req, res, next) => {
    try {
...
router.get('/patient/stats', auth_1.authenticate, async (req, res, next) => {
    try {
        const patient = await prisma_1.default.patient.findUnique({
            where: { userId: req.user?.userId }
        });
        ...
        const stats = {
            ...
            active: permissions.filter(p => p.status === 'approved' &&
                (!p.expiresAt || new Date(p.expiresAt) > now)).length
        };
        res.json(stats);
    }
    catch (error) {

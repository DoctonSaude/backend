...
class IntelligenceService {
     ...
     */
    async checkEconomyOpportunities(userId) {
        logger_js_1.logger.info(`[IntelligenceService] Checking economy opportunities for user ${userId}`);
        const user = await prisma_js_1.default.user.findUnique({
            where: { id: userId },
            include: {
                person: {
        ...
        for (const sub of subscriptions) {
            ...
            const similarProducts = await prisma_js_1.default.pharmacyProduct.findMany({
                where: {
                    OR: [
            ...
            const activePromotions = await prisma_js_1.default.pharmacyPromotion.findMany({
                where: {
            ...
            let currentPrice = 0;
            if (sub.pharmacyId) {
                const currentProduct = await prisma_js_1.default.pharmacyProduct.findFirst({
                    where: {
        ...
        for (const insight of economyInsights) {
            await aiInsight_service_js_1.aiInsightService.createInsight({
                ...insight,
                userId: userId
            });
            await inAppNotification_service_js_1.default.createNotification({

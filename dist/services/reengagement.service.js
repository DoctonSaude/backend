...
class ReengagementService {
    ...
    static async processPredictiveReplenishment() {
        ...
        try {
            ...
            const stats = await prisma_js_1.default.userProductStats.findMany({
                where: {
                    isRecurring: true,
            ...
            for (const stat of stats) {
                try {
                    ...
                    if (!stat.lastPurchaseDate || !(stat.lastPurchaseDate instanceof Date))
                        continue;
                    if (!stat.averageIntervalDays || !isFinite(stat.averageIntervalDays) || stat.averageIntervalDays <= 0)
                        continue;
                    if (!stat.userId)
                        continue;
     ...
     */
    static async sendPersonalizedAlert(stat, daysLeft) {

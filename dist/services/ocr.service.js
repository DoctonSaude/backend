...
const drug_matching_service_js_1 = require("./drug-matching.service.js");
class OCRService {
    drugMatchingService;
    constructor() {
    ...
    async processImage(params) {
        ...
        try {
            ...
            ocrProcessing = await prisma_js_1.default.oCRProcessing.create({
                data: {
            ...
            const updatedProcessing = await prisma_js_1.default.oCRProcessing.update({
            ...
            // Atualizar com erro
            if (ocrProcessing) {
                await prisma_js_1.default.oCRProcessing.update({
                    where: { id: ocrProcessing.id },
                    data: {
                        status: 'FAILED',
     ...
     */
    async saveProcessedImage(imageBuffer, ocrId) {
    ...
    async preprocessImage(imageBuffer) {
        const config = pedomed_config_js_1.PEDOMED_CONFIG.OCR.PROCESSING;
    ...
    async extractText(imageBuffer) {
        ...
        const worker = await (0, tesseract_js_1.createWorker)(languages, 1, {
            logger: (m) => {
    ...
    async validateImage(imageBuffer, mimeType) {
        // Verificar tamanho
        if (imageBuffer.length > pedomed_config_js_1.PEDOMED_CONFIG.OCR.MAX_FILE_SIZE) {
            throw new Error(`Image too large. Maximum size: ${pedomed_config_js_1.PEDOMED_CONFIG.OCR.MAX_FILE_SIZE / 1024 / 1024}MB`);
        }
        // Verificar formato
        ...
        const { MIN_IMAGE_WIDTH, MIN_IMAGE_HEIGHT, MAX_IMAGE_WIDTH, MAX_IMAGE_HEIGHT } = pedomed_config_js_1.PEDOMED_CONFIG.OCR.QUALITY_CHECKS;
     ...
     */
    async saveOriginalImage(imageBuffer, filename) {
        const uploadsDir = (0, path_1.join)(pedomed_config_js_1.PEDOMED_CONFIG.OCR.STORAGE.LOCAL_PATH, 'original');
        await fs_1.promises.mkdir(uploadsDir, { recursive: true });
        const finalFilename = `${Date.now()}_${filename}`;
        const filePath = (0, path_1.join)(uploadsDir, finalFilename);
        await fs_1.promises.writeFile(filePath, imageBuffer);
     ...
     */
    async extractDrugs(text) {
        const drugs = [];
        const config = pedomed_config_js_1.PEDOMED_CONFIG.OCR.DRUG_EXTRACTION;
        if (!config.ENABLED) {
            return drugs;
        }
        ...
        for (const pattern of config.PATTERNS) {
            ...
            while ((match = regex.exec(text)) !== null) {
                ...
                if (config.BLACKLIST.some((word) => drugName.toLowerCase().includes(word.toLowerCase()))) {
                    continue;
                }
                ...
                if (config.USE_DRUG_MATCHING) {
                    try {
                        ...
                        if (searchResults.length > 0) {
                            ...
                            confidence = Math.min(confidence, searchResults[0].confidence);
                        }
                    }
                    catch (error) {
        ...
        return drugs.slice(0, pedomed_config_js_1.PEDOMED_CONFIG.OCR.VALIDATION.MAX_DETECTED_DRUGS);
    }
    /**
     ...
     */
    async getProcessingById(id, userId) {
        // TODO: Implementar quando Prisma client for regenerado
     ...
     */
    async confirmDetectedDrug(detectedDrugId, userId) {
        // TODO: Implementar quando Prisma client for regenerado
     ...
     */
    async rejectDetectedDrug(detectedDrugId, userId) {
        // TODO: Implementar quando Prisma client for regenerado
